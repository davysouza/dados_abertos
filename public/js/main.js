jQuery(document).ready(function($){
	...
	$form_login.find('input[type="email"]').on('change', function(event){
		$form_login.find('input[type="email"]').removeClass('has-error').next('span').removeClass('is-visible');
		event.preventDefault();
		var filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
		var sEmail = $('#signin-email').val();
		if(!filter.test(sEmail)) {
			$form_login.find('input[type="email"]').addClass('has-error').next('span').addClass('is-visible');
		}
	});
	$form_signup.find('input[type="email"]').on('change', function(event){
